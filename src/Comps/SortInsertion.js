...
export default function SortInsertion(props) {
    ...
    const swapper = () => {
        let i, j, res = [], key, swap = false;
        res = processedArray.length > 0 ? processedArray : [...unsorted.current];
        for (i = 0; i < res.length; i++) {

            key = res[i].value;
            j = i - 1;
            while (j >= 0 && res[j].value > key) {
                res[j + 1].value = res[j].value;
                j = j - 1;
                swap = true;
            }
            res[j + 1].value = key;

            let el = document.querySelector("p[id=" + CSS.escape(j) + "]");
